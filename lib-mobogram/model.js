...
var messageTypes = {
  ...
  "video_note": function (msg, options) {
    if (msg.video_note === undefined) return false;
    var video_note = checkObj(msg.video_note);
    delete msg.video_note;

    this.file = new File().parseLight(video_note, options);

    this.duration = checkInt(video_note.duration);
    delete video_note.duration;

    if (video_note.thumb !== undefined) {
      this.thumbnail = new Image().parse(video_note.thumb, options);
      delete video_note.thumb;
    }
    ...
    utils.verifyEmpty(video_note, options);
    return true;
  },
